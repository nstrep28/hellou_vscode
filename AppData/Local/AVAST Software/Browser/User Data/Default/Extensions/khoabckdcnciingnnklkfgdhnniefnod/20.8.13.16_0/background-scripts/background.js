...
/**
 * Check if Avast HTTPS Encryption should be ON for host
 */
...
function updateState () {
  ...
  chrome.browserAction.setTitle({
    title: 'Avast HTTPS Encryption' + ((iconState === 'active') ? '' : ' (' + iconState + ')')
  });
    ...
    if (isExtensionDisabledOnSite(hostname) || iconState == "disabled") {
      if ('setIcon' in chrome.browserAction) {
        chrome.browserAction.setIcon({
          path: {
            38: `images/icons/icon-disabled-${window.matchMedia('(prefers-color-scheme: dark)').matches ? "dark-" : ""}38.png`
          }
        });
      }
    } else {
...
function onBeforeRequest(details) {
  ...
  if (details.type == "main_frame") {
    ...
    // Check if an user has disabled Avast HTTPS Encryption on this site.  We should
    // ensure that all subresources are not run through Avast HTTPS Encryption as well.
